#include "/home/whchoi/vispark/tutorial/Volume_Rendering/helper_math.h"
...
__device__ int slider_opacity1[4];
...
public:
    ...
    GPU line_iter(float3 from, float3 to, float d, float depth) {
    	S = from;
		E = to;
...
template<typename R, typename T> GPU R cut_plane_query(T* image, float2 p, float step, float3 normal, float3 point, VIVALDI_DATA_RANGE* vdr) {
	...
	float upper = dot((point - ray_origin), normal), lower = dot(ray_direction, normal);
	float dist;
	...
	return point_query_3d<R>(image, sample_point, 0, vdr);
}

template<typename R, typename T> GPU R cut_plane_query(T* image, float x, float y, float step, float3 normal, float3 point, VIVALDI_DATA_RANGE* vdr) {
	return cut_plane_query<R>(image, make_float2(x,y), step, normal, point, vdr);
}
...
template<typename R,typename T> GPU T cubic_query_3d(T* volume, float3 p, int border, VIVALDI_DATA_RANGE* sdr){
	...
	// 8 linear queries
	R q000 = linear_query_3d<R>(volume, p.x - hgx.x, p.y - hgy.x, p.z - hgz.x, border, sdr);
	R q001 = linear_query_3d<R>(volume, p.x - hgx.x, p.y - hgy.x, p.z + hgz.y, border, sdr);
	R q010 = linear_query_3d<R>(volume, p.x - hgx.x, p.y + hgy.y, p.z - hgz.x, border, sdr);
	R q011 = linear_query_3d<R>(volume, p.x - hgx.x, p.y + hgy.y, p.z + hgz.y, border, sdr);
	R q100 = linear_query_3d<R>(volume, p.x + hgx.y, p.y - hgy.x, p.z - hgz.x, border, sdr);
	R q101 = linear_query_3d<R>(volume, p.x + hgx.y, p.y - hgy.x, p.z + hgz.y, border, sdr);
	R q110 = linear_query_3d<R>(volume, p.x + hgx.y, p.y + hgy.y, p.z - hgz.x, border, sdr);
	R q111 = linear_query_3d<R>(volume, p.x + hgx.y, p.y + hgy.y, p.z + hgz.y, border, sdr);
...
template<typename R, typename T> GPU R cubic_query_3d(T* volume, float3 p, VIVALDI_DATA_RANGE* sdr){
	return cubic_query_3d<R>(volume, p, 0, sdr);
}
template<typename R, typename T> GPU R cubic_query_3d(T* volume, float x, float y, float z, int border, VIVALDI_DATA_RANGE* sdr){
	return cubic_query_3d<R>(volume, make_float3(x,y,z), border, sdr);
}
template<typename R, typename T> GPU R cubic_query_3d(T* volume, float x, float y, float z, VIVALDI_DATA_RANGE* sdr){
	return cubic_query_3d<R>(volume, make_float3(x,y,z), 0, sdr);
}
...
//	return line_iter(make_float3(val), make_float3(val,val,val+1), 1.0);
	...
	if(!(interval.x < interval.y)) return line_iter(make_float3(0), make_float3(0), 1.0, -9999);
    float3 S = ray_origin + ((int)(interval.x/step)*step) * ray_direction;
    float3 E = ray_origin + ((int)(interval.y/step-3)*step)  * ray_direction;

	return line_iter(S,E,step, interval.y);
}
...
__device__ float4 alpha_compositing(float4 origin, float4 next)
{
	...
	x = r + (1-a/255) * next.x * next.w/255.0;
	y = g + (1-a/255) * next.y * next.w/255.0;
	z = b + (1-a/255) * next.z * next.w/255.0;
...
__device__ float4 alpha_compositing(float4 origin, float4 next, float aa)
{
	...
	if(!isfinite(w/w_p) && !isfinite(w_p/w)) {
		x = r + (1-a/255.0) * next.x * next.w/255.0;
		y = g + (1-a/255.0) * next.y * next.w/255.0;
		z = b + (1-a/255.0) * next.z * next.w/255.0;
		return make_float4(x, y, z, w);
	}

	x = r*aa + (1-aa*a/255.0) * next.x * next.w/255.0;
	y = g*aa + (1-aa*a/255.0) * next.y * next.w/255.0;
	z = b*aa + (1-aa*a/255.0) * next.z * next.w/255.0;
...
__device__ float4 transfer(float a)
{
	...
	//float4 tmp_col = make_float4(tmp.x*255.0, tmp.y*255.0, tmp.z*255.0, a/TF_bandwidth*255);
...
texture<float4, 2> TFF5;
__device__ float4 transfer(float a, int chan)
{
	float4 tmp;
	if(chan == 0) 
		//tmp = tex2D(TFF, a/TF_bandwidth * 255, 0);
		//tmp = tex2D(TFF, (a-100)/(TF_bandwidth-100) * 255, 0);
		tmp = tex2D(TFF,  a/TF_bandwidth * 255, 0);
	else if(chan == 1) 
		//tmp = tex2D(TFF1, (a-100)/(TF_bandwidth-100) * 255, 0);
		...
		tmp = tex2D(TFF5, a/TF_bandwidth * 255, 0);
...
extern "C"{
__global__ void mipshort(result_dtype* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, short* volume, VIVALDI_DATA_RANGE* volume_DATA_RANGE, int x_start, int x_end, int y_start, int y_end){
    ...
    z_val = line_iter.get_depth();
        rb[(x-rb_DATA_RANGE->buffer_start.x)+(y-rb_DATA_RANGE->buffer_start.y)*(rb_DATA_RANGE->buffer_end.x-rb_DATA_RANGE->buffer_start.x)] = result_dtype(x,y,z_val,max);
    return;
}
