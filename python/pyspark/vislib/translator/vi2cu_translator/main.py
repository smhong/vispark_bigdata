import os
import sys
path = "%s/vislib/translator"%os.getenv('PYSPARK_SRC')
...
def change_return(code_list, local_dict):
    ...
    for code in code_list:
        ...
        flag = True
        idx1 = -1
        global return_class, lambda_func
        return_class = ""
        lambda_func = ""
        while flag:
            flag = False
    ...
    #               after = after[:idx3+1] + indent + 'return\n' + after[idx3+1:] 
                    ...
                    ####
                    global comma_cnt
                    comma_cnt = var_name.count(',')
                    if comma_cnt > 0:
                        return_class = return_class + "class result_dtype{\npublic:\n\tfloat val[%d];\n\tGPU result_dtype("%(comma_cnt+1)
    
                        for elem in range(comma_cnt+1):
                            if elem < 1:
                                return_class = return_class + "int a%d,"%(elem)
                            else:
                                return_class = return_class + "float a%d,"%(elem)
                        return_class = return_class[:-1] + "){"
                        for elem in range(comma_cnt+1):
                            return_class = return_class + "\n\t\tval[%d]=a%d;"%(elem, elem)
                        return_class = return_class + "\n\t}\n};"
    
                        lambda_func = "itertools.imap(lambda ("
                        for elem in range(comma_cnt+1):
                            lambda_func = lambda_func + "a%d,"%(elem)
                        ...
                        for elem in var_name:
                            if elem in ['(', '[']:
                                tmp_func = tmp_func + elem
                                open_cnt = open_cnt + 1
                            elif elem in [')', ']']:
                                close_flag = True
                                if tmp_cnt > 0:
                                    tmp_func = tmp_func + "a%d"%local_cnt + elem
                                    tmp_cnt -= 1
                                else:
                                    tmp_func = tmp_func + elem
                                ...
                                if close_flag == False and tmp_cnt > 0:
                                    tmp_func = tmp_func + "a%d"%local_cnt + elem
                                    local_cnt = local_cnt + 1
                                    tmp_cnt -= 1
                                else:
                        ...
                        lambda_func = lambda_func + tmp_func + ", result_buffer)"
                                
                        
                        global return_dtype
                        local_dict['rb'] = 'result_dtype'
                        return_dtype = find_dtype(var_name, local_dict)
                        return_dtype = to_CUDA_dtype(return_dtype)
                        
                        after_tmp = "result_dtype("
                        for elem in after[:idx3+1].strip():
                            if elem not in ('[','(', ')', ']', '\n', ' '):
                                after_tmp = after_tmp + elem
                        ####
                    ...
                    num = idx2 + len(mid) + idx3 + len(indent) + 10
                    idx1 = num + 1
    ...
    return code_list, return_class, lambda_func
...
#               code_list.append(indent + '    ' + var + ' = ' + elem_list[3] + '.next()\n')
        ...
        # change return
        code_list, return_class, lambda_func = change_return(code_list, local_dict)
        # change function
...
def add_make(source):
    ...
    source = local_source
    for elem in source:
        if elem.find('float2') >= 0:
            ret.append(elem.replace('float2','make_float2'))
        elif elem.find('float3') >= 0:
            ret.append(elem.replace('float3','make_float3'))
        elif elem.find('float4') >= 0:
            ret.append(elem.replace('float4','make_float4'))
        elif elem.find('int2') >= 0:
            ret.append(elem.replace('int2','make_int2'))
        elif elem.find('int3') >= 0:
            ret.append(elem.replace('int3','make_int3'))
        elif elem.find('int4') >= 0:
            ret.append(elem.replace('int4','make_int4'))
        elif elem.find('double2') >= 0:
            ret.append(elem.replace('double2','make_double2'))
        elif elem.find('double3') >= 0:
            ret.append(elem.replace('double3','make_double3'))
        elif elem.find('double4') >= 0:
            ret.append(elem.replace('double4','make_double4'))
...
def parse_body(vivaldi_code_body='', local_dict={}):
    ...
    # change python dtype to cuda dtype
    CUDA_body = add_make(source=vivaldi_code_body)

    # Parse block
    #
    #######################################################
    # parse Vivaldi code body to CUDA body
    CUDA_body, inner_dict = parse_block(block=CUDA_body, local_dict=local_dict)
...
def vi2cu_translator(vivaldi_code='', local_dict={}):
    ...
    _vc = vivaldi_code.split('\n')
    
    vivaldi_code = ''
    for elem in _vc:
        if elem.replace(' ', '').find('#') == -1:
            vivaldi_code = vivaldi_code + elem + '\n'
...
#   print "Z",local_dict
    ...
    global return_class, lambda_func, comma_cnt
    return CUDA_code, comma_cnt, return_class, lambda_func
