...
def load_GPU_attachment():
    import os
    src_path = os.getenv('PYSPARK_SRC')
    includes = '#include "'+src_path+'/vislib/translator/helper_math.h"'+'\n'
    attachment = includes + read_file('%s/vislib/translator/Vivaldi_cuda_attachment.cu'%os.getenv('PYSPARK_SRC'))
    attachment += 'extern "C"{\n'
