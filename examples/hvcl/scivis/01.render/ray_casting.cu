#include "/home/freyja/Private/vivaldi/Vivaldi_Project/Vivaldi/src/interactive_mode/helper_math.h"
...
//__global__ void render(float* rb, uchar* volume, VIVALDI_DATA_RANGE* volume_DATA_RANGE, int width, int height, float4* transfer_func, float* inv_model, int xoff, int yoff) {
__global__ void render(float* rb, float* volume, VIVALDI_DATA_RANGE* volume_DATA_RANGE, int width, int height, float4* transfer_func, float* inv_model, int xoff, int yoff) {
    ...
    line_iter = orthogonal_iter(volume, idx_x+xoff, idx_y+yoff, 1.0, volume_DATA_RANGE);
    ...
    for(float3 elem = line_iter.begin(); line_iter.valid(); elem = line_iter.next()){
		...
		float val_diff = val - floor(val);
		//val = val_diff * ceil(val) + (1-val_diff) * 
		float4 top_col = transfer_func[(int)ceil(val)];
		float4 bot_col = transfer_func[(int)floor(val)];
	
		tmp_val = top_col * (1-val_diff) + bot_col * val_diff;
		//tmp_val = make_float4(val,val,val,val);
...
__global__ void composite(unsigned char* output, float* input, int num_img, int dim_x, int dim_y) {
    int idx_x = threadIdx.x + blockDim.x * blockIdx.x;
    int idx_y = threadIdx.y + blockDim.y * blockIdx.y;
	...
	//		printf("cnt : %d | R: %f, G: %f, B: %f\n", i, r, g, b);
		...
		result = alpha_compositing(result, curr_col);
    }
