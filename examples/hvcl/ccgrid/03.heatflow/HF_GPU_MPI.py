...
def send_count(name):
    t0 = time.time()
    print name, "Send Count Start : %0.3f"%(time.time())
    msg_size = 4*1024
    ...
    print name, "Send Count End: %0.3f"%(time.time())
...
def shuffle_halo(name):
    ...
    print name, "Send Shuffle Start : %0.3f"%(time.time())
    msg_size = 4*1024
    ...
    print name, "Send Shuffle End : %0.3f"%(time.time())
    #print "%d process : %0.3f"%(pid,time.time()-t1)
...
def genhalo3d(name,data,InfoPack):
    ...
    t0 = time.time()
    print name, "Halo Gen Start : %0.3f"%(time.time())
    ...
    print name, "Halo Gen End: %0.3f"%(time.time())
...
def attachHalo3d(name,data,InfoPack):
    print name, "Halo Attach Start: %0.3f"%(time.time())
    ...
    def recv_data(target_loc, target_key, target_split,shape):
        ...
        return numpy.fromstring(target_data,dtype=numpy.uint8).reshape(shape)
    ...
    print name, "Halo Attach End: %0.3f"%(time.time())
...
def heatfunc(name,data,InfoPack,cuda_code):
    t0 = time.time()
    print name, "GPU Start: %0.3f"%(time.time())
    ...
    h_A = cuda.pagelocked_empty(data.shape,dtype=data.dtype)
    d_A = cuda.mem_alloc(data.nbytes)
   
    h_B = cuda.pagelocked_empty(data.shape,dtype=data.dtype)
    d_B = cuda.mem_alloc(data.nbytes)
   
    h_A[:] = data

    cuda.memcpy_htod_async(d_A,h_A,stream)

    def get_block_grid(nx, ny, nz):
        block = [8,8,8]
        grid  = [1,1,1]
    
        import math  
 
        grid[0] = int(math.ceil(nx/8.0))
        grid[1] = int(math.ceil(ny/8.0))
        grid[2] = int(math.ceil(nz/8.0))
    ...
    block, grid = get_block_grid(nx,ny,nz)
 
    cuda_args = [d_B, d_A, numpy.int32(nx), numpy.int32(ny), numpy.int32(nz), numpy.int32(nx), numpy.int32(ny)]

    mod = SourceModule(cuda_code, no_extern_c = True, arch="sm_52" ,options = ["-use_fast_math", "-O3", "-w"])

    cuda_func = mod.get_function("heatf") 
    ...
    cuda.memcpy_dtoh_async(h_B,d_B,stream)
    stream.synchronize()

    result = numpy.array(h_B).reshape(data.shape) 

    print name, "GPU End: %0.3f"%(time.time())

    return (name,result.astype(numpy.uint8))
...
def init_raw(name,data,InfoPack):
    ...
    #return (name,data) 
    return (name,data.astype(numpy.uint8)) 
...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName="HF_Spark_GPU_"+str(ImageName))
    ...
    ImagePatch = sc.binaryFiles(ImagePath, tag="VISPARK")
    ...
    # Raw  
        ...
        for ii in range(5):
            ...
            ImageHalo = ImagePatch.newRDD()

            print ImagePatch, ImagePatch.rdd()
            print ImageHalo, ImageHalo.rdd()

            ImageHalo = ImageHalo.map(lambda (name,data): genhalo3d(name,data,InfoPack))
            ImageHalo.foreach (lambda (key):send_count(key)) 

            print ImagePatch, ImagePatch.rdd()
            #print ImagePatch.collect()
            ...
            #ImagePatch = ImagePatch.vmap(heatf(data, Ori_Size_X, Ori_Size_Y, Ori_Size_Z, Ori_Size_X).range(x=0:514,y=0:514,z=0:258).output(float, 68162568).extern_code(cuda_code))
            ImagePatch = ImagePatch.map(lambda (key,data): heatfunc(key,data,InfoPack,cuda_code))
            #ImagePatch = ImagePatch.vmap(heatf(data, 514, 258, 258, 514, 258).range(x=0:514,y=0:258,z=0:258).output(float, 1).extern_code(cuda_code))
            #ImagePatch = ImagePatch.map(lambda (key,data): (key,numpy.fromstring(data,dtype=numpy.float32).reshape((DataDim[2],DataDim[1],DataDim[0]))))
