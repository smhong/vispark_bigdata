...
if __name__ == "__main__":
    ...
    elems = int(sys.argv[1]) if len(sys.argv)>=2 else 10000000
    isPersist = int(sys.argv[2]) if len(sys.argv)>=3 else 1
    #dType = int(sys.argv[3]) if len(sys.argv)>=4 else 1
#    global D
    D = int(sys.argv[3]) if len(sys.argv)>=4 else 40
    ...
    DataPath='hdfs://emerald1:9000/user/whchoi/lr_%s_%s'%(elems,D)
    sc = SparkContext(appName="PythonLR_for_%d_%d"%(elems,D))
    ...
    w = 2 * numpy.random.ranf(size=D) - 1
    ...
    def gradient(matrix, w):
        ...
        print ((1.0 / (1.0 + numpy.exp(-Y * X.dot(w))) - 1.0))
        return ((1.0 / (1.0 + numpy.exp(-Y * X.dot(w))) - 1.0) * Y * X.T).sum(1)
